...
void main() {
  ...
  group('laod entries and sorted', () {
    ...
    test('check if a before b', () {
      expect(x[0].startTime.millisecondsSinceEpoch <= x[1].startTime.millisecondsSinceEpoch, true);
      expect(x[3].startTime.millisecondsSinceEpoch <= x[4].startTime.millisecondsSinceEpoch, true);
      expect(x[9].startTime.millisecondsSinceEpoch <= x[9].startTime.millisecondsSinceEpoch, true);
      expect(x[8].startTime.millisecondsSinceEpoch <= x[9].startTime.millisecondsSinceEpoch, true);
      expect(x[7].startTime.millisecondsSinceEpoch <= x[5].startTime.millisecondsSinceEpoch, false);
      expect(x[0].startTime.millisecondsSinceEpoch <= x[9].startTime.millisecondsSinceEpoch, true);
    });
  });
}
